  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (password.length < 5) {
        setError('Mật khẩu tối thiểu 5 ký tự');
        setLoading(false);
        return;
      }
      // Demo: giả lập đăng ký thành công, sau đó auto đăng nhập
      await new Promise((r) => setTimeout(r, 700));
      setSuccess('Đăng ký thành công. Đang tự động đăng nhập...');
      const result = await signIn('credentials', {
        email: email || 'demo@yggdrasil.io',
    ...
    <div className="relative w-full max-w-md scroll-reveal visible">
      ...
      <div className="relative p-[2px] rounded-2xl animated-gradient">
        ...
        <div className="text-center mb-8">
          ...
          <p className="text-slate-500 mt-2">Gia nhập phong trào chống tẩy xanh</p>
        </div>

        {/* Tabs */}
        <div className="flex gap-2 mb-6 bg-slate-100 rounded-full p-1">
          <Link href="/login" className="flex-1 text-center py-2 text-slate-600 hover:text-slate-900 rounded-full font-medium transition">Đăng Nhập</Link>
          <div className="flex-1 text-center py-2 bg-teal-500 text-white rounded-full font-medium">Đăng Ký</div>
        </div>
